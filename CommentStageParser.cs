...
namespace DataStreamProcess.Processing.Units.CommentParsers
{
    public class CommentStageParser
    {
        ...
        public void PerformAllTypeMatcher()
        {
            ...
            //1-ий этаж, 2-ой этаж
            MatchCollection st1 = Regex.Matches(comment, @"([\d]+)[ \s-ыйои–]+этаж[ \s!.,-;]+?");
            //этаж 1,  этаж первый
            ...
            MatchCollection st2 = Regex.Matches(comment, @"(?<![\d]{1,2}[ ]{1,})этаж[ :\-\s]+([\d]{1,2}|[первый]{4,}|[второй]{4,}|[третий]{4,}|[подземный]{4,}|[цокольный]{4,}|[четвертый]{4,}|[пятый]{4,}|[шестой]{4,}|[седьмой]{4,}|[восьмой]{4,}|[девятый]{4,}|[десятый]{4,})[ :.!,\s;]+?");

            //этаж 1/9 (этаж 1)
            MatchCollection st3 = Regex.Matches(comment, @"этаж[ \s\-]+[\d](?=[\/][\d])");
            //на 1/9 (возвращает на 1)
            MatchCollection st4 = Regex.Matches(comment, @"на[ \s]+[\d](?=[\/][\d])");
            //первый этаж, второй этаж
            MatchCollection st5 = Regex.Matches(comment, @"([первый]{4,}|[второй]{4,}|[третий]{4,}|[подземный]{4,}|[цокольный]{4,}|[цокольное]{4,}|[четвертый]{4,}|[пятый]{4,}|[шестой]{4,}|[седьмой]{4,}|[восьмой]{4,}|[девятый]{4,}|[десятый]{4,})[ .!,\s;]+?(этаж|этаже)[ !;.,:\s]+");
            // in instead of on
            ...
            //первого этажа
            MatchCollection st10 = Regex.Matches(comment, @"([первого]{4,}|[второго]{4,}|[третьего]{4,}|[подземного]{4,}|[цокольного]{4,}|[цокольное]{4,}|[четвертого]{4,}|[пятого]{4,}|[шестого]{4,}|[седьмого]{4,}|[восьмого]{4,}|[девятого]{4,}|[десятого]{4,})[ .!,\s;]+?(этажа)[ !;.,:\s]+");
            ...
            this.FindNumericStages(st7); this.FindNumericStages(st9);
